...
const { HotModuleReplacementPlugin, webpack, DefinePlugin } = require("webpack");
...
module.exports = {
  ...
  plugins: [
        ...
        const stats = JSON.stringify(
          {
            scripts: Object.entries(assets).flatMap(([_asset, files]) => {
              return files.filter((filename) => (filename.endsWith(".js") && !/\.hot-update\./.test(filename)) || (filename.endsWith(".tsx") && !/\.hot-update\./.test(filename)));
            }),
    ...
    new DefinePlugin({
      "process.env": {
        IS_OFFLINE: JSON.stringify(process.env.IS_OFFLINE),
      },
    }),
  ].filter(Boolean),
  module: {
    rules: [
      {
        //include all files with a `.ts` or `.tsx` extension in src path
        test: /\**\.(ts|tsx)$/,
